...
SingleEntryVector(i::Int, n::Int, v::N) where {N} = SingleEntryVector{N}(i, n, v)

function Base.getindex(x::SingleEntryVector{N}, i::Int) where {N}
    @boundscheck 1 <= i <= x.n || throw(BoundsError(x, i))
    return i == x.i ? x.v : zero(N)
end

Base.size(x::SingleEntryVector) = (x.n,)

# negation
function Base.:(-)(x::SingleEntryVector)
...
for (opS, opF) in ((:(+), +), (:(-), -))
    @eval begin
        function Base.$opS(x1::SingleEntryVector, x2::SingleEntryVector)
            @assert x1.n == x2.n ||
...
function Base.:(*)(D::Diagonal, x::SingleEntryVector)
...
function LinearAlgebra.norm(x::SingleEntryVector, p::Real=2)
