...
function _logarithmic_norm_1(A::AbstractMatrix{N}) where {N}
    out = -Inf
...
function _logarithmic_norm_inf(A::AbstractMatrix{N}) where {N}
    out = -Inf
...
# max_j  1/2 * λⱼ(A + Aᵀ)
function _logarithmic_norm_2(A::AbstractMatrix)
    B = A + A'
    λ = eigmax(B)
    return λ / 2
end
